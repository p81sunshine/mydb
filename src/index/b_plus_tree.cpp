...
BPlusTreeInternalPage *BPlusTree::Split(InternalPage *node, Transaction *transaction) {
  ...
  GenericKey *key_pass_to_parent = node->KeyAt(node->GetSize() / 2);
...
//BPlusTreePage *BPlusTree::Split_new(BPlusTreePage *node, Transaction *transaction) {
//  if (node->IsLeafPage()) {
//    ...
//  } else {
//    ...
...
void BPlusTree::Remove(const GenericKey *key, Transaction *transaction) {
  if (IsEmpty()) return;
  auto leaf_page = reinterpret_cast<LeafPage *>(FindLeafPage(key));
  GenericKey *old_first_key = leaf_page->KeyAt(0);
  int size_after_delete = leaf_page->RemoveAndDeleteRecord(key, processor_);
  if (size_after_delete != -1) {
    GenericKey *new_first_key = leaf_page->KeyAt(0);
    if (processor_.CompareKeys(old_first_key, new_first_key) != 0 &&
        !leaf_page->IsRootPage()) {  // 检查删完后第一个pair是否发生了改变，如果改变，要更新父节点(如果不是root）
      auto parent_page =
          reinterpret_cast<InternalPage *>(buffer_pool_manager_->FetchPage(leaf_page->GetParentPageId()));
...
bool BPlusTree::CoalesceOrRedistribute(N *&node, Transaction *transaction) {
  if (node->IsRootPage()) {
    if (node->GetSize() < 2) {
      if (AdjustRoot(node)) return true;
    }
    return false;
  }
  ...
  if (node_index != 0 && node_index != parent_page->GetSize() - 1) {
    auto prev_sibling =
        reinterpret_cast<N *>(buffer_pool_manager_->FetchPage(parent_page->ValueAt(node_index - 1)));
    auto next_sibling =
        reinterpret_cast<N *>(buffer_pool_manager_->FetchPage(parent_page->ValueAt(node_index + 1)));
    if (prev_sibling->GetSize() > prev_sibling->GetMinSize()) {
      ...
      if (node->IsLeafPage()) {  // node是叶结点的情况
        ...
        auto *temp_page = reinterpret_cast<LeafPage *>(FindLeafPage(nullptr, next_sibling->GetPageId(), true));
        GenericKey *key = temp_page->KeyAt(0);
        buffer_pool_manager_->UnpinPage(temp_page->GetPageId(), false);
...
IndexIterator BPlusTree::Begin() {
  ...
  page_id_t page_id = FindLeafPage(nullptr, root_page_id_, true)->GetPageId();
...
Page *BPlusTree::FindLeafPage(const GenericKey *key, page_id_t page_id, bool leftMost) {
  ...
  if (!leftMost) {
    while (!page->IsLeafPage()) {
      ...
      next_page_id = page->Lookup(key, processor_);
